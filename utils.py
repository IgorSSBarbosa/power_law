...
def compute_power_law_exponent(
        dom,
        X_n,
        plot,
        rho,
    ...
    log_n = np.emath.logn(rho,dom)
    log_X_n = np.emath.logn(rho,X_n)
    ...
    if plot:
        ...
        ax.plot(dom, np.pow(rho,intercept) * (dom ** slope), 'r--', 
        ...
        ax.set(title=f'Log-Log plot of {simulation}')
        ax.legend()

        ax.grid()
        ax.grid(which='minor', color="0.9")
        save_plot_path = Path(f'images/log-log_plots/{model}/plot_{simulation}.png')
